...
const getAll = async (): Promise<IThac0sRow[]> => {
  const [rows] = await pool.execute<IThac0sRowDataPacket[]>("SELECT * FROM thac0s;");
  return rows;
};

const getById = async (id: number): Promise<IThac0sRow | null> => {
  const [rows] = await pool.execute<IThac0sRowDataPacket[]>(`SELECT * FROM thac0s WHERE id = ?;`, [id]);
  return rows.length > 0 ? rows[0] : null;
};

const getByCharacterLevel = async (characterLevel: number): Promise<IThac0sRow | null> => {
  const [rows] = await pool.execute<IThac0sRowDataPacket[]>(`SELECT * FROM thac0s WHERE character_level = ?;`, [
    characterLevel,
  ]);
  return rows.length > 0 ? rows[0] : null;
};
